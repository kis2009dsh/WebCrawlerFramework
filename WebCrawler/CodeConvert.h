...
#ifndef __WebCrawler__CodeConvert__
#define __WebCrawler__CodeConvert__
...
public:
    ...
    std::string Convert(const std::string& input ) throw (code_convert_exception)
    {
        ...
        if( ret == size_t(-1) )
        {
            throw code_convert_exception(getErrorMsg(ret));
        }
...
private:
    ...
    static inline
    const char * getErrorMsg(size_t retCode)
    {
        switch (errno) {
            case E2BIG:
...
#endif /* defined(__WebCrawler__CodeConvert__) */
